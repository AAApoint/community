...
public class DiscussPostController implements CommunityConstant{
    ...
    public String addDiscussPost(String title, String content){
        ...
        if(user == null){
            return CommunityUtil.getJSONString(403, "您还没有进行登录！");
        }
        ...
        // 报错情况将来统一处理
        return CommunityUtil.getJSONString(0, "上传帖子成功！");
    }

    @RequestMapping(path = "/detail/{discussPostId}", method = RequestMethod.GET)
    public String getPostDetailPage(@PathVariable("discussPostId") int discussPostId, Model model, Page page){
        DiscussPost discussPost = discussPostService.getDiscussPostDetail(discussPostId);
        ...
        int likeStatus = hostHolder.getUser() == null ? 0 :
                likeService.getLikeStatus(hostHolder.getUser().getId(), ENTITY_TYPE_POST, discussPost.getId());
        ...
        if(commentList != null){
            for (Comment comment : commentList){
                ...
                long commentLikeNum = likeService.getLikeNum(2, comment.getId());
                commentVO.put("commentLikeNum", commentLikeNum);
                ...
                for (Comment reply : replyList){
                    ...
                    long replyLikeNum = likeService.getLikeNum(2, reply.getId());
                    replyVO.put("replyLikeNum", replyLikeNum);
