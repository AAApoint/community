...
//        session.setAttribute("kaptcha", text);
    ...
    public String login(String username, String password, String code, boolean rememberMe,
                      Model model/*, HttpSession session*/, HttpServletResponse response, @CookieValue("owner") String owner){
        // 检查验证码
//        String kaptcha = (String) session.getAttribute("kaptcha");
        ...
        // 检查账号、密码
        long expired_seconds = rememberMe ? REMEMBER_EXPIRED_SECONDS : DEFAULT_EXPIRED_SECONDS;
        Map<String, Object> map = userService.login(username, password, expired_seconds);
        if(map.containsKey("ticket")){
            Cookie cookie = new Cookie("ticket", map.get("ticket").toString());
            cookie.setPath(contextPath);
            cookie.setMaxAge((int)expired_seconds);
            response.addCookie(cookie);
